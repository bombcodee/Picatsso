...
4. `docs/MEMORY.md`는 마일스톤 완료 시 자동 업데이트 + "메모리에 저장해줘" 요청 시에도 업데이트
...
9. 사용자가 여러 질문/요청을 한 번에 보냈을 때, 답변하지 못한 항목이 있으면 반드시 다시 체크해서 빠짐없이 응답할 것. 놓친 질문이 없는지 마지막에 스스로 검증할 것
10. **커밋 전 문서 정합성 검증 (필수):**
    - 커밋 요청 시, 관련 문서(README, ARCHITECTURE, DATA_FLOW, API_SPEC, MEMORY 등)가 현재 코드와 일치하는지 확인
    - 불일치하는 부분이 있으면 사용자에게 알리고 최신화
    - 위험 요소(깨진 참조, 삭제된 파일 언급, 잘못된 스펙 등)가 있으면 경고 + 승인 요청
    - 마일스톤 완료 시에도 동일하게 문서 동기화 수행
