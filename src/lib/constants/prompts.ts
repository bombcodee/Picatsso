 ...
 *
 * === 감정 표현 (파레이돌리아 효과 + 실제 존재 시 강조) ===
 *
 * [규칙 1: 장면에 실제로 있는 경우]
 * 좋아하는 것이 장면에 실제로 존재하면 → 선명한 파란색으로 확실하게, 빛나듯이 그린다.
 * 싫어하는 것이 장면에 실제로 존재하면 → 탁한 회색/갈색으로 흐릿하고 불쾌하게 그린다.
 *
 * [규칙 2: 장면에 없는 경우 — 파레이돌리아]
 * 좋아하는 것({loves})이 장면에 없으면 직접 그리지 않는다.
 ...
 * 싫어하는 것({dislikes})이 장면에 없으면 직접 그리지 않는다.
